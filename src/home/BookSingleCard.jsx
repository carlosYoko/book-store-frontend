...
const BookSingleCard = ({ book }) => {
  ...
  return (
    ...
    >
      <h2 className="absolute top-1 right-2 px-4 py-1 bg-slate-500 rounded-lg">
        {book.publishYear}
      </h2>
      ...
      <div className="flex justify-between items-center gap-x-2 mt-4 p-4">
        ...
        <Link to={`/api/books/details/${book._id}`}>
          <BsInfoCircle className="text-2xl text-green-600 hover:text-slate-300" />
        </Link>
        <Link to={`/api/books/edit/${book._id}`}>
          <AiOutlineEdit className="text-2xl text-yellow-600 hover:text-slate-300" />
        </Link>
        <Link to={`/api/books/delete/${book._id}`}>
          <MdOutlineDelete className="text-2xl text-red-600 hover:text-slate-300" />
        </Link>
      </div>
      {showModal && (
